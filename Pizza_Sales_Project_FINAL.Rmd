...
pizza_sales$date <- ymd(pizza_sales$date)
pizza_sales$day_of_week <- wday(pizza_sales$date, label = TRUE, abbr = FALSE)

pizza_sales$day_of_week <- as.factor(pizza_sales$day_of_week)

model <- lm(quantity ~ time + price + day_of_week, data = pizza_sales)
...
set.seed(42)
sampled_data <- pizza_sales %>%
  sample_frac(0.1) 

sampled_data$normalized_price <- scale(sampled_data$price)
sampled_data$normalized_quantity <- scale(sampled_data$quantity)

features_for_clustering <- sampled_data %>%
  select(normalized_price, normalized_quantity, size)  # Include the "size" variable


set.seed(42)
wss <- sapply(1:10, function(k) {
  kmeans(features_for_clustering[, c("normalized_price", "normalized_quantity")], centers = k)$tot.withinss
})
...
# Run K-means clustering with a smaller batch size
set.seed(42)
final_kmeans <- kmeans(features_for_clustering[, c("normalized_price", "normalized_quantity")], centers = 3, iter.max = 100, nstart = 25)

# Analyzing the cluster output
sampled_data$cluster <- final_kmeans$cluster
cluster_summary <- sampled_data %>%
  group_by(cluster) %>%
  summarise(avg_price = mean(price),
            avg_quantity = mean(quantity),
            avg_size = table(size) %>% as.data.frame() %>% slice(which.max(Freq)) %>% pull(size))
...
#Based on the cluster analysis model:



Cluster 1: Price-Conscious Customers (Small Pizzas)
Average Price: $14.193
Average Quantity: 1.0000
Average Size: Medium (M)

Marketing Strategy:
Emphasize affordability and value for money for smaller-sized pizzas (Medium).
Offer budget-friendly deals and promotions targeting customers who prefer smaller pizzas.
Highlight quality and taste of smaller pizzas to appeal to price-conscious customers.



Cluster 2: Premium Segment (Large Pizzas)
Average Price: $20.496
Average Quantity: 1.0000
Average Size: Large (L)

Marketing Strategy:
Focus on premium quality and dining experience, especially for larger-sized pizzas (Large).
Offer premium toppings or gourmet options for customers seeking a premium pizza experience.
Highlight exclusivity and quality ingredients to attract customers willing to pay a premium.

Cluster 3: Moderate Spenders with Quantity (Large Pizzas)
Average Price: $16.963
Average Quantity: 2.0256
Average Size: Large (L)

Marketing Strategy:
Provide value for larger orders, particularly for larger-sized pizzas (Large).
Offer bundle deals and upsizing options for customers ordering multiple pizzas or larger-sized pizzas.
Highlight convenience and variety for group dining or family meals with larger-sized pizzas.

By tailoring marketing strategies to each cluster's preferences regarding price, quantity, and pizza size, businesses can effectively target their messaging and promotions to different customer segments, maximizing customer engagement and satisfaction.
